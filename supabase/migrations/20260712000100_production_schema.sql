...
-- Security correction: secret-bearing raw round rows are visible only after
-- results. Active state is available through get_round_for_player_v2.
...
-- Security correction: raw votes expose only the caller's own vote, completed
-- results, or Shared-Device votes to the current Shared-Device host.
CREATE POLICY "Participants can view authorized votes"
ON public.votes FOR SELECT TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM public.round_participants caller_rp
    JOIN public.players caller_player ON caller_player.id = caller_rp.player_id
    JOIN public.rounds vote_round ON vote_round.id = votes.round_id
    JOIN public.rooms vote_room ON vote_room.id = vote_round.room_id
    WHERE caller_rp.round_id = votes.round_id
      AND caller_player.user_id = (SELECT auth.uid())
      AND (
        vote_round.phase = 'results'
        OR votes.voter_player_id = caller_player.id
        OR (
          vote_room.game_mode = 'local'
          AND private.is_current_host(vote_room.id, (SELECT auth.uid()))
        )
      )
  )
);
